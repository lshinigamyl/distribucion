...
$nombre = "";
 if(empty(test_input($_POST['nombre']))){
  ...
  $nombre=$_POST['nombre'];
  $consulta="select id,nombre,telefono,domicilio,ciudad from proveedores where nombre like '%$nombre%' order by ".$_GET['jtSorting'] ." limit " . $_GET['jtStartIndex'] . ',' . $_GET['jtPageSize'] . "";
  $consulta2="Select COUNT(*) AS RecordCount from proveedores";
  }
...
$trayendo=$db->consulta($consulta);
   ...
   while($row=mysqli_fetch_assoc($trayendo)){
     $rows[]=$row;
   }
        $jTableResult = array();
