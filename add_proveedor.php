...
$create="insert into proveedores(nombre,telefono,domicilio,ciudad) values
('$nombre','$telefono','$domicilio','$ciudad')";
$ejecuta=$db->consulta($create);

$ultimoid="SELECT * FROM proveedores WHERE id = LAST_INSERT_ID();";
$exe=$db->consulta($ultimoid);

        $jTableResult = array();
		$jTableResult['Result'] = "OK";
        $jTableResult['Record'] = mysqli_fetch_array($exe);
